...
class Slider {
  constructor(props) {
    ...
    this.slides = Array.from(
      this.rootElement.querySelectorAll(".slider__list--item")
    );
    ...
    this.thumbs = Array.from(this.rootElement.querySelectorAll(".slider__nav-control"));
    this.prevButton = this.rootElement.querySelector(".slider__arrow-prev");
    this.nextButton = this.rootElement.querySelector(".slider__arrow-next");

    this.slides[this.current].classList.add("slider__list--item_active");
    this.thumbs[this.current].classList.add("slider__nav-control_active");
  ...
  goTo(index, dir) {
    ...
    self.current = index;
    nextSlide.classList.add("slider__list--item_active");
    ...
    anime(Object.assign({}, self.baseAnimeSettings, {
      ...
      complete: function(anim) {
        ...
        self.thumbs.forEach((item, index) => {
          ...
          item.classList[action]("slider__nav-control_active");
        });
      }
    }))
  ...
  _navClickHandler(e) {
    ...
    let target = e.target.closest(".slider__nav-control");
    if (!target) return;
